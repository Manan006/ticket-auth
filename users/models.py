...
class UserManager(BaseUserManager):
  ...
  def create_superuser(self, email, password, **extra_fields):
    user=self._create_user(email=email, password=password, is_staff=True, is_superuser=True, **extra_fields)
    user.save(using=self._db)
    return user
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    campus_id = models.PositiveSmallIntegerField(null=True, blank=False)
    course_id = models.PositiveSmallIntegerField(null=True, blank=False)
